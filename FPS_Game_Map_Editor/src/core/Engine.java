...
public class Engine
{
	...
	private void loadMap()
	{
		...
		for( int i = 0; i < map.length; i++ ) for( int j = 0; j < map[0].length; j++ ) for( int k = 0; k < map[0][0].length; k++ ) map[i][j][k] = -128;
	...
	private void mainLoop()
	{
		...
		//	display transparent rect for plane
		glColor3f( 0, 0, 0 );
		glBegin( GL_QUADS );
		glVertex3d( -5000, -testPlayer.yPos - 150, -5000 );
		glVertex3d( -5000, -testPlayer.yPos - 150, 5000 );
		glVertex3d( 5000, -testPlayer.yPos - 150, 5000 );
		glVertex3d( 5000, -testPlayer.yPos - 150, -5000 );
		glEnd();
